...
| 2 | Архитектура и поверхность атаки | Описаны компоненты, границы доверия и 23 API-endpoint'а; статическая инвентаризация выполнена, динамическая проверка прав вынесена в пункт 10 | `PASS` | [Матрица endpoint'ов](./evidence/02-attack-surface/endpoints.md); выполнить role/object negative tests в пункте 10 |
...
| 8 | Автоматический DAST | ZAP Baseline frontend выявил 2 Medium, 6 Low и 2 Informational alert'а; backend проверен по трём GET-endpoint'ам, единственный LOW alert классифицирован как false positive | `FINDING` | [Frontend](./evidence/08-dast/zap-frontend-report.md), [backend](./evidence/08-dast/zap-backend-report.json); вручную проверить frontend-заголовки, остальной API — в пунктах 9–10 |
...
| 11 | Верификация и оценка риска | Результаты дедуплицированы, false positive отделены, подтверждённые находки получили CWE/OWASP, confidence и приоритет риска | `PASS` | [Реестр подтверждённых находок](#реестр-подтверждённых-находок); перед ретестом дополнить полный CVSS 4.0 vector для High/Critical |
...
1. Перед продолжением аудита передать владельцу системы закрытое уведомление о `SECRET-IMG-01`, не раскрывая значение ключа; получить подтверждение отзыва и безопасные сведения об IAM/журналах.
2. Завершить оставшуюся часть пакета 5: выполнить SCA backend и сформировать SBOM.
3. Исправить подтверждённые BOLA/BFLA-находки и добавить автоматические regression-тесты для anonymous/cross-user доступа и state-changing операций.
4. Завершить оставшиеся сценарии пунктов 9–10: авторизованную загрузку, token lifecycle, rate limiting и дополнительные manager/admin/object проверки.
5. Выполнить пункт 11: провести дедупликацию, назначить CWE и полный CVSS 4.0 vector; затем заменить этот срез итоговым отчётом. Всё непокрытое сохранить как `BLOCKED` или `NOT TESTED`.
