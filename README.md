...
**Дата среза:** 19 августа 2026 года. **Охват:** выполнены и документированы 6 из 12 запланированных пакетов. Этот раздел является промежуточным отчётом и не заменяет итоговый отчёт после завершения оставшихся проверок и триажа.

Проверка подтвердила работоспособность локального стенда и позволила зафиксировать его идентичность, поверхность атаки, доступную конфигурацию CI/CD, результаты поиска секретов, SCA frontend, автоматического SAST frontend/backend и ручного security review backend. Наиболее существенные подтверждённые результаты — включение GCP service-account JSON в backend-образ, детерминированная генерация authentication tokens, отсутствие их реализованной инвалидации и вывод административного пароля в журнал. Значение найденного GCP-ключа не извлекалось и не проверялось на внешних системах. Дополнительно выявлены уязвимые frontend-зависимости, изменяемые ссылки на образы и GitHub Actions, публикация портов на всех интерфейсах и запуск backend-контейнера без директивы `USER`.

Выводы ограничены фактически сохранёнными доказательствами. Пункты 1–6 выполнялись в указанном объёме; пункты 7–12 ещё не выполнялись и имеют статус `NOT TESTED`. Пункт 5 остаётся частично завершённым: SCA frontend выполнен, SCA backend и формирование SBOM ещё предстоят. Пункт 6 завершён автоматическим SAST и ручным review и имеет общий статус `FINDING`; кандидаты на нарушения авторизации будут проверены динамически в пункте 10. Закрытые настройки GitHub, действительность найденного ключа, его IAM-права и журналы GCP недоступны в текущем наборе доказательств; их состояние не считается проверенным.
...
| 7 | Контейнеры и конфигурация | Не выполнялось как отдельный пакет | `NOT TESTED` | Выполнить Trivy image/config и review `inspect/history`, root/capabilities/FS/healthcheck |
...
| 12 | Рекомендации и следы выполнения | Выполнено только предварительно в этом срезе | `NOT TESTED` | Завершить после пунктов 7–11 и сформировать итоговую coverage-матрицу |
...
2. Завершить пакет 4: повторить полный Trivy scan frontend-образа и сохранить воспроизводимый санитаризированный Gitleaks-результат с версией инструмента, точной командой, временем и target commit SHA.
3. Завершить оставшуюся часть пакета 5 (SCA backend и SBOM) и параллельно выполнить пакеты 7–8: container/config и локальный ZAP Baseline.
