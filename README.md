...
Выводы ограничены фактически сохранёнными доказательствами. Пункты 5–12 ещё не выполнялись и имеют статус `NOT TESTED`. Закрытые настройки GitHub, действительность найденного ключа, его IAM-права и журналы GCP недоступны в текущем наборе доказательств; их состояние не считается проверенным.
...
| 5 | Зависимости и SBOM | Не выполнялось | `NOT TESTED` | Запустить `npm audit` и Trivy filesystem, проверить применимость CVE и fixed versions |
