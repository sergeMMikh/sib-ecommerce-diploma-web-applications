...
| Требование продукта |	Что проверяем |
|------|------|
| Пользователь должен видеть только свои заказы	| Авторизация, IDOR |
| Пароль должен храниться безопасно |	Аутентификация |
| API должно быть защищено	| Проверка API |
| Конфиденциальные данные не должны попадать в журналы	| Анализ логов |
| Контейнеры должны безопасно разворачиваться |	Docker |
...
#### Полный план выполнения проверок

Интервалы ниже относятся к будущему этапу выполнения. Автоматические проверки секретов, зависимостей, кода и образов запускаются параллельно; поэтому их календарное время меньше простой суммы длительностей.
