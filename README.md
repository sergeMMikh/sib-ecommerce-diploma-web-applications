...
| 2 | Архитектура и поверхность атаки | Описаны компоненты, границы доверия и 23 API-endpoint'а; динамическая проверка фактических прав не проводилась | `FINDING` | [Матрица endpoint'ов](./evidence/02-attack-surface/endpoints.md); выполнить role/object negative tests в пункте 10 |
...
| 4 | Секреты | В backend-образе подтверждён `/src/fcm.json`; Gitleaks-проверка истории описана, frontend image scan не завершён | `FINDING` | [Санитаризированный Trivy-результат](./evidence/04-secrets/trivy-image-secret-scan.txt); завершить frontend scan и сохранить воспроизводимый Gitleaks-артефакт |
