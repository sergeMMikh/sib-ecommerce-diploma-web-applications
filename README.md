...
Наличие alert'а ZAP является сигналом для проверки, а не автоматическим доказательством самостоятельной эксплуатируемой уязвимости. Наиболее значимые результаты относятся к отсутствующим защитным HTTP-заголовкам и должны быть подтверждены ручной проверкой ответов nginx. Сканирование backend API на `127.0.0.1:9999` в представленном наборе evidence отсутствует и сохраняет статус `NOT TESTED`.
...
**Итог пункта 8:** `FINDING` для frontend — baseline выявил отсутствующие защитные заголовки и раскрытие версии сервера. Проверка API ещё не выполнена, поэтому охват пакета остаётся частичным.
...
| 8 | Автоматический DAST | ZAP Baseline frontend выявил 2 Medium, 6 Low и 2 Informational alert'а; High alert'ов нет; backend API не сканировался | `FINDING` | [Отчёт ZAP frontend](./evidence/08-dast/zap-frontend-report.md); вручную проверить заголовки и выполнить baseline API |
...
3. Завершить пакет 8: вручную подтвердить frontend alert'ы и выполнить OWASP ZAP Baseline для backend API.
