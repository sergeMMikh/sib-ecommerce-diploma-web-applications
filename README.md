...
#### Нормативно-превовая база

В рамках данной работы не проводится сертификация приложения на соответствие требованиям законодательства или стандартов. Указанные документы используются как источник требований информационной безопасности, а проверка ограничивается анализом реализованных механизмов защиты.

Главный для ИБ- 152-ФЗ «О персональных данных». Интернет-магазин почти наверняка обрабатывает ФИО, телефон, e-mail, адрес доставки, данные учётной записи и историю заказов. Закон прямо требует принимать правовые, организационные и технические меры против неправомерного доступа, изменения, копирования и распространения персональных данных, а также контролировать доступ и регистрировать действия с ними.

Закон РФ № 2300-1 «О защите прав потребителей», особенно ст. 26.1 о дистанционной продаже. Она регулирует именно покупки через интернет: покупателю должна предоставляться информация о товаре и продавце, а также условия возврата; закон устанавливает и право отказа от дистанционно приобретённого товара.

54-ФЗ о применении ККТ. При интернет-расчётах продавец в соответствующих случаях должен формировать и передавать покупателю электронный кассовый чек, например на телефон или e-mail, предоставленные покупателем. Требования 54-ФЗ идентифицированы как потенциально применимые к промышленной эксплуатации сервиса, однако проверка интеграции с ККТ не проводится, поскольку такая функциональность отсутствует в исследуемом стенде.

161-ФЗ. Требования законодательства о национальной платёжной системе учитываются только в случае непосредственной реализации приложением функций приёма или обработки платежей.

PCI DSS- это вообще не закон, а отраслевой стандарт. Но если приложение принимает данные банковских карт, он становится очень важным источником security requirements. Если же магазин использует внешнюю платёжную страницу и PAN/CVV через backend не проходят, область PCI DSS резко уменьшается.
...
Для данной работы считаю достаточным список соответсвия 152-ФЗ, 54-ФЗ и 2300-1 о защите прав потребителей.
...
| Источник | Требование продукта |	Что проверяем |
|------|------|------|
| 152-ФЗ |	защита персональных данных	| авторизация, контроль доступа |
| ASVS	| безопасная аутентификация	| анализ кода + ручное тестирование |
| WSTG	| проверка API	| DAST |
| CIS Docker	| безопасность контейнеров |	Docker audit |
| | | |
...
| № | Что проверяется | Как проверяется: подходы и инструменты | База | Ожидаемый артефакт |
|---:|---|---|---|---|
...
| 2 | Архитектура и поверхность атаки | По контроллерам, frontend-запросам, DevTools и ZAP составляется матрица «метод — маршрут — роль — объект — данные» и схема доверительных границ | WSTG 4.1, ASVS, API Security Top 10 | Карта компонентов, ролей, маршрутов, заказов, телефонов, токенов и файлов |
| 3 | Репозитории и процесс разработки | Проверяются workflows, тесты, pinning Actions, `permissions`, правила веток/rulesets, два review, security jobs, Dependabot, Secret Scanning и происхождение образов; GitHub UI/API, OpenSSF Scorecard | NIST SSDF, OWASP SAMM, SLSA | Матрица заявленных и подтверждённых SDLC-контролей; недоступное — `BLOCKED` |
| 4 | Секреты | Gitleaks с `--redact` по рабочему дереву и полной Git-истории, доступный GitHub Secret Scanning и ручной анализ конфигураций/артефактов | ASVS, NIST SSDF | Только маскированные координаты и fingerprint потенциальных секретов |
| 5 | Зависимости и SBOM | `npm audit --json`, Trivy filesystem; при прогретой базе — OWASP Dependency-Check для Gradle. Проверяются runtime/dev scope, CVE, применимость и исправленная версия | OWASP Top 10:2025, ASVS, NIST SSDF | SBOM и проверенный перечень применимых уязвимых компонентов | 
| 6 | Исходный код | Semgrep CE `--config auto`; вручную проверяются security config, auth filter, controllers/services, работа с объектами, токенами, файлами и логами; HIGH/CRITICAL перепроверяются | ASVS 5.0.0, CWE, OWASP Top 10:2025 | Санитаризированные SAST-результаты с воспроизведением или отметкой false positive | 
| 7 | Контейнеры и конфигурация | Trivy image/config, `docker inspect/history`; проверяются mutable tags, base images, root, capabilities, writable FS, секреты в слоях, healthcheck, CORS, debug/TRACE, ошибки и HTTP-заголовки | CIS Docker, Docker best practices, ASVS, WSTG 4.2 | Перечень применимых рисков образов, runtime и конфигурации | 
| 8 | Автоматический DAST | OWASP ZAP Baseline для frontend и API; ограниченные активные правила только на локальном стенде с лимитом запросов. Из контейнера ZAP используется `host.docker.internal` | WSTG, ASVS, OWASP Top 10:2025 | HTML/JSON-отчёт ZAP и журнал охваченных адресов | 
| 9 | Ручные проверки web/API | ZAP/Burp Community, DevTools и `curl`: валидация, HTTP-методы, ошибки, заголовки, CORS, XSS/инъекции, загрузка/выдача файлов, товары, заказы и комментарии | WSTG 4.2, ASVS, API Security Top 10 | Набор воспроизводимых request/response-сценариев без реальных данных |
...
| 11 | Верификация и оценка риска | Удаляются дубликаты и false positive, находки воспроизводятся, получают CWE, OWASP-категорию, confidence, бизнес-влияние и CVSS 4.0 score с vector string | CWE, OWASP, CVSS 4.0 | Реестр подтверждённых находок с единообразным приоритетом | 
| 12 | Рекомендации и следы выполнения | Для каждой находки описываются маскированное доказательство, риск, исправление, первичный источник и критерий ретеста; формируется coverage-матрица | ASVS, WSTG, NIST SSDF, первичные документы поставщиков | Материалы для итогового отчёта и перечень `PASS/FINDING/BLOCKED/NOT TESTED` |
| | | |
