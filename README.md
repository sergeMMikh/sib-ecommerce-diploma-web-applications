...
1. Подготовка и проверка тестового окружения

Цель: развернуть исследуемое приложение в изолированной локальной среде и убедиться в его работоспособности перед проведением динамического анализа.

Планируемые действия:

* изучить предоставленные репозитории frontend и backend;
* проанализировать файлы сборки и конфигурацию Docker Compose;
* проверить используемые контейнерные образы, их версии и источники;
* запустить frontend и backend с помощью Docker Compose;
* проверить доступность приложения и его API;
* определить открытые порты и схему сетевого взаимодействия контейнеров;
* проанализировать журналы запуска на наличие ошибок и чувствительной информации;
* зафиксировать параметры тестового окружения и версии используемых компонентов.

Инструменты: Docker, Docker Compose, команды docker ps, docker inspect, docker logs, curl, браузерные инструменты разработчика.

Ожидаемый результат: функционирующий локальный тестовый стенд, пригодный для проведения статического, динамического и ручного анализа безопасности.

2.	Определение архитектуры и поверхности атаки	Анализ frontend, backend, API, портов, маршрутов и механизмов аутентификации
3.	Анализ репозиториев и процесса разработки	GitHub Actions, настройки веток, Code Review, автоматические проверки
4.	Поиск секретов	Gitleaks или TruffleHog, GitHub Secret Scanning, ручной анализ
5.	Анализ зависимостей	Dependabot, npm audit, Gradle/Maven-инструменты, OWASP Dependency-Check
6.	Статический анализ кода	Semgrep, SonarQube или SonarCloud, ручная проверка критичных участков
7.	Анализ контейнеров	Trivy, Dockerfile best practices, проверка пользователя, тегов и пакетов
8.	Проверка конфигурации приложения	Переменные окружения, CORS, debug-режим, обработка ошибок, заголовки
9.	Динамическое сканирование	OWASP ZAP, passive scan, active scan в локальном стенде	
10.	Ручное тестирование веб-приложения и API	OWASP WSTG, Burp/ZAP, DevTools, curl/Postman
11.	Проверка аутентификации и авторизации	Подмена идентификаторов, разграничение ролей, сессии, JWT, brute force
12.	Анализ результатов и оценка рисков	CVSS, CWE, OWASP Top 10, исключение ложных срабатываний	
13.	Подготовка рекомендаций и отчёта	Описание доказательств, рисков, способов устранения и повторной проверки	
