...
**Дата среза:** 16 августа 2026 года. **Охват:** выполнены и документированы 4 из 12 запланированных пакетов. Этот раздел является промежуточным отчётом и не заменяет итоговый отчёт после завершения оставшихся проверок и триажа.

Проверка подтвердила работоспособность локального стенда и позволила зафиксировать его идентичность, поверхность атаки, доступную конфигурацию CI/CD и результаты первичного поиска секретов. Наиболее существенный подтверждённый результат — включение GCP service-account JSON в файловую систему backend-образа. Значение ключа не извлекалось и не проверялось на внешних системах. Одновременно выявлены изменяемые ссылки на образы, публикация портов на всех интерфейсах, избыточно подробные backend-журналы и недостаточно подтверждённые меры безопасности процесса разработки.

Выводы ограничены фактически сохранёнными доказательствами. Пункты 1–5 выполнялись в указанном объёме; пункты 6–12 ещё не выполнялись и имеют статус `NOT TESTED`. Пункт 5 остаётся частично завершённым: SCA frontend выполнен, SCA backend и формирование SBOM ещё предстоят. Закрытые настройки GitHub, действительность найденного ключа, его IAM-права и журналы GCP недоступны в текущем наборе доказательств; их состояние не считается проверенным.
...
| 6 | Исходный код | Не выполнялось | `NOT TESTED` | Запустить Semgrep и вручную проверить auth/authz, файлы, валидацию и логирование |
...
| 12 | Рекомендации и следы выполнения | Выполнено только предварительно в этом срезе | `NOT TESTED` | Завершить после пунктов 5–11 и сформировать итоговую coverage-матрицу |
...
3. Параллельно выполнить пакеты 5–8 в установленном time-box: SCA/SBOM, SAST, container/config и локальный ZAP Baseline.
