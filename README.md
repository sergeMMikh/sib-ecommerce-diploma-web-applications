...
**Дата среза:** 20 августа 2026 года. **Охват:** выполнялись и документированы 8 из 12 запланированных пакетов. Этот раздел является промежуточным отчётом и не заменяет итоговый отчёт после завершения оставшихся проверок и триажа.

Проверка подтвердила работоспособность локального стенда и позволила зафиксировать его идентичность, поверхность атаки, доступную конфигурацию CI/CD, результаты поиска секретов, SCA frontend, автоматического SAST frontend/backend, ручного security review backend, проверки контейнеров/runtime-конфигурации и ZAP Baseline frontend/backend. Наиболее существенные подтверждённые результаты — включение GCP service-account JSON в backend-образ, детерминированная генерация authentication tokens, отсутствие их реализованной инвалидации, вывод административного пароля в журнал, большое количество HIGH/CRITICAL findings в обоих контейнерных образах и избыточный состав backend image. Значение найденного GCP-ключа не извлекалось и не проверялось на внешних системах.

Выводы ограничены фактически сохранёнными доказательствами. Пункты 1–8 выполнялись в указанном объёме; пункты 9–12 ещё не выполнялись и имеют статус `NOT TESTED`. Пункт 5 остаётся частично завершённым: SCA frontend выполнен, SCA backend и формирование SBOM ещё предстоят. Пункт 6 завершён автоматическим SAST и ручным review и имеет общий статус `FINDING`; кандидаты на нарушения авторизации будут проверены динамически в пункте 10. Пункт 7 завершён как отдельный пакет и имеет статус `FINDING`. В пункте 8 завершены ZAP Baseline frontend и ограниченный baseline трёх backend GET-endpoint'ов; прочие API routes и методы вынесены в пункты 9–10. Закрытые настройки GitHub, действительность найденного ключа, его IAM-права и журналы GCP недоступны в текущем наборе доказательств; их состояние не считается проверенным.
...
| 6 | Исходный код | Semgrep проверил frontend и backend; ручной review подтвердил детерминированные authentication tokens, отсутствие их инвалидации и вывод административного пароля в лог; нарушения авторизации для заказов и удаления товаров остаются кандидатами до динамической проверки | `FINDING` | [Ручной security review](./evidence/06-source-code/manual-security-review.md); подтвердить authz-кандидаты в пункте 10 |
...
| 9 | Ручные проверки web/API | Не выполнялось | `NOT TESTED` | Выполнить ограниченные негативные сценарии с тестовыми данными |
| 10 | Аутентификация, сессии и авторизация | Не выполнялось | `NOT TESTED` | Проверить матрицу ролей и объектов с disposable accounts |
...
| 12 | Рекомендации и следы выполнения | Выполнено только предварительно в этом срезе | `NOT TESTED` | Завершить после пунктов 9–11 и сформировать итоговую coverage-матрицу |
...
| P2 | Часть потенциально опасных API-маршрутов не имеет явной роли на уровне контроллера | Средняя, гипотеза до DAST/manual verification | Карта endpoint'ов и `anyRequest().permitAll()` определяют приоритет проверки, но не доказывают эксплуатацию | Выполнить role/object matrix и подтвердить корректный `401/403` либо воспроизводимую BOLA/BFLA-находку |
...
3. Вручную подтвердить frontend alert'ы пункта 8; расширенные API-сценарии выполнять в рамках пунктов 9–10.
4. По карте поверхности атаки выполнить пакеты 9–10 только с тестовыми учётными записями и локальными адресами `127.0.0.1:8888`/`127.0.0.1:9999`.
5. После воспроизведения результатов провести дедупликацию, назначить CWE и полный CVSS 4.0 vector и заменить этот срез итоговым отчётом. Всё непокрытое сохранить как `BLOCKED` или `NOT TESTED`.
